...
def process_excel_returns(file_path: str | Path) -> Path | None:
    ...
    Проверяет формат файла. Ищет заголовки, собирает данные,
    группирует суммы по ключу ("Ключевое поле", "Дата последнего возврата").
    Оставляет первую найденную строку для каждой уникальной группы, суммирует
    в ней возвраты, а значения в ячейках дублирующих строк очищает (оставляет пустые строки).

    Args:
        file_path (str | Path): Абсолютный путь к исходному файлу Excel.
